...
typedef void(__cdecl* AIO_LOG_CALLBACK)(int eventType, DWORD processId, LPCWSTR message);
AIO_LOG_CALLBACK logCallback = nullptr;
...
constexpr auto DriverServiceName = "netfilter2";

bool EnsureDriverInstalled()
{
	auto manager = OpenSCManagerA(nullptr, nullptr, SC_MANAGER_CONNECT);
	if (manager == nullptr)
	{
		printf("[WinProcessProxy][driver] OpenSCManager failed: %lu\n", GetLastError());
		return false;
	}

	auto service = OpenServiceA(manager, DriverServiceName, SERVICE_QUERY_STATUS);
	if (service != nullptr)
	{
	...
	{
		printf("[WinProcessProxy][driver] OpenService failed: %lu\n", error);
		return false;
	}

	auto status = nf_registerDriver(DriverServiceName);
	if (status != NF_STATUS_SUCCESS)
	{
		printf("[WinProcessProxy][driver] nf_registerDriver failed: %d\n", status);
		return false;
	}
...
extern "C" {
	__declspec(dllexport) void __cdecl aio_setLogCallback(AIO_LOG_CALLBACK callback)
	{
	...
	__declspec(dllexport) BOOL __cdecl aio_register(LPWSTR value)
	{
		...
		if (status != NF_STATUS_SUCCESS)
		{
			printf("[WinProcessProxy][aio_register] nf_registerDriver: %d\n", status);
			return FALSE;
		}
	...
	__declspec(dllexport) BOOL __cdecl aio_unregister(LPWSTR value)
	{
		...
		if (status != NF_STATUS_SUCCESS)
		{
			printf("[WinProcessProxy][aio_unregister] nf_unRegisterDriver: %d\n", status);
			return FALSE;
		}
	...
	__declspec(dllexport) BOOL __cdecl aio_dial(int name, LPWSTR value)
	{
		...
		case AIO_BYPNAME:
			try
			{
				std::wregex checker(value, regex_constants::icase);
			}
			catch (regex_error) {
				return FALSE;
			}
	...
	__declspec(dllexport) BOOL __cdecl aio_init()
	{
		...
		{
			puts("[WinProcessProxy][aio_init] WSAStartup != NO_ERROR");
			return FALSE;
		}

		if (!eh_init())
		{
			puts("[WinProcessProxy][aio_init] !eh_init");
			WSACleanup();
			return FALSE;
		}
		...
		{
			puts("[WinProcessProxy][aio_init] nf_init != NF_STATUS_SUCCESS");
			eh_free();
			WSACleanup();
			return FALSE;
		}
